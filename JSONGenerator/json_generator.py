...
def read_trace(trace_id, index, trace_dict):
    if trace_id in trace_dict.keys():
        trace_elements = trace_dict[trace_id].split("\t")
        return trace_elements[index]


def process_in_out(in_out_file, trace_dict):
    outputs = {}
    tasks = []
    for in_out_line in in_out_file:
        in_out_elements = in_out_line.split()
        hash_id = in_out_elements[ID_SCRIPT]
        cpu_usage = float(read_trace(hash_id, AVERAGE_CPU_USAGE, trace_dict).replace('%', '')) / 100

        task = Task(
            hash_id,
            read_trace(hash_id, TASK_NAME, trace_dict),
            int(float(read_trace(hash_id, N_CYCLES, trace_dict).replace(',', '.'))),
            10_000,
            cpu_usage,
            math.ceil(cpu_usage)
        )
...
def main():
    parser = argparse.ArgumentParser(description='Generate the Wrench Workflow JSON ')
    parser.add_argument('trace', type=str, help='path to trace.txt file')
    parser.add_argument('iofile', type=str, help='path to the task io file')
    parser.add_argument('--output', dest='output', type=str, help='path to the task io file', default='workflow.json')
    ...
    with open(args.iofile) as script:
        input_output_file = script.readlines()

    print("Starting JSON generation...")
    result_json = process_in_out(input_output_file, trace_dict)
